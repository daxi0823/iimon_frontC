    ...
    <main>
      <section id="mv" class="mv__less">
        <div class="copy__wrapper">
          ...
          <v-btn @click="imgUpload">アップロード</v-btn>
...
export default {
  ...
  methods: {
    ...
    addMenu() {
      addDoc(collection(db, "menus"), {
        ...
        .then((doc) => {
          ...
          this.menuTxt = "";
          this.image = [];
          // const menuImgUrlRemain = document.getElementById("imgId");
    ...
    imgUpload() {
      ...
      //画像ファイルへの参照を作成
      const userImageRef = ref(storage, `images/${this.image.name}`);
      
      //画像ファイルのアップロードメソッド
      uploadBytesResumable(userImageRef, this.image).then((snapshot) => {
        console.log("Uploaded a blob or file!", snapshot);
        getDownloadURL(snapshot.ref)
          .then((downloadURL) => {
            //firestoreにURLとファイル名を保存するため
            this.menuImgUrl = downloadURL;
            this.menuImgFile = this.image.name;
            console.log("Success!",this.image.name, downloadURL);
          })
          .catch((error) => {
            console.error(error);
          });
